...
- If the fix **depends on** a `refactor:` commit that isn't on master yet, land
  that refactor on master first via a temp branch, then rebase the issue branch
  onto the updated master before cherry-picking:
...
otherwise). Stripping `wip:` removes the imperative verb that satisfied
conform's imperative-mood check, so the derived subject has to stand on its own:
...
route — issue #1005 stayed open because its PR body was empty. Recover with:
