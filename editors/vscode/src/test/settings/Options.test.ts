...
import { ClassCheckMode } from "../../main/settings/ClassCheckMode";
import { Options } from "../../main/settings/Options";
...
suite("Options", () => {
  ...
  test("apply adds protected class methods flag when enabled", () => {
    const args = collectArgs({ checkProtectedClassMethods: true });
    assert.deepEqual(args, ["--check-protected-class-methods"]);
  });
