...
protected methods, and property returns, will be checked, even though
they aren't enabled by default when running the program.
...
def test_multiple(
    ...
    Only run tests on templates prefixed with ``M``, as these are
    designated templates containing 2 or more functions. These templates
    are generally excluded from other tests.
...
def test_ignore_no_params(
    ...
    missing from the docstring. These should not trigger with this
    argument.
...
def test_single_flag(
    ...
    """Test that failing templates pass with only a corresponding flag.
